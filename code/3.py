...
def cross_validation_experiment(drug_dis_matrix, drug_matrix, dis_matrix, seed, epochs, emb_dim, dp, lr, adjdp):
    ...
    all_predict_y_probas_concatenated = np.concatenate(all_predict_y_probas, axis=0)
    # 计算平均 ROC 曲线和 AUC
    mean_fpr, mean_tpr, _ = roc_curve(
        drug_dis_matrix.flatten(), all_predict_y_probas_concatenated.flatten())
    mean_auc = auc(mean_fpr, mean_tpr)

    # 绘制 ROC 曲线
    plt.figure(figsize=(8, 6))
    plt.plot(mean_fpr, mean_tpr, color='b', linestyle='--', label='平均 ROC (AUC = %0.2f)' % mean_auc)
    plt.xlabel('假阳性率')
    plt.ylabel('真阳性率')
    plt.title('受试者工作特性 (ROC)')
