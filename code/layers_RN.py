...
class GraphConvolution():
    ...
    def __call__(self, inputs):                         #定义了当调用图卷积层对象时要执行的操作
        with tf.name_scope(self.name):
            ...
            # neighbor_aggregation = tf.sparse_tensor_dense_matmul(self.adj, x)  # 聚合邻居节点的特征
            # combined = tf.concat([x, neighbor_aggregation], axis=1)  # 将原始节点特征与聚合后的邻居特征拼接在一起
            # x = tf.matmul(combined, self.vars['weights'])  # 使用权重矩阵进行线性变换
