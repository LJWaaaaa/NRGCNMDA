...
from layers_RN import GraphConvolution,GraphConvolution2,GraphConvolutionSparse, InnerProductDecoder
from utils import *
...
class ResidualBlock3(tf.keras.layers.Layer):
    def __init__(self, emb_dim,adj,dropout,act):
        ...
        self.conv3 = GraphConvolution2(
            name='gcn_residual_layer3',
            input_dim=emb_dim,
            output_dim=emb_dim,
            adj=self.adj,
            dropout=self.dropout,
    ...
    def call(self, inputs):
        ...
        residual = x
        x = self.conv1(x)
        x = self.batch_norm(x)  # 在第一个图卷积层后添加归一化

        x = self.conv2(x)
        x = self.batch_norm(x)  # 在第二个图卷积层后添加归一化
...
class GCNModel():
    ...
    def build(self):
        ...
        # self.residual_block3 = ResidualBlock2(emb_dim=self.emb_dim, adj=self.adj, dropout=self.dropout, act=self.act)
        # self.residual3 = self.residual_block3(self.residual2)

        # self.layer3 = tf.keras.layers.Dense(units=32, activation=tf.nn.relu)
        # R_branch_2 = self.layer3(self.residual2)
        # self.residual_block3 = ResidualBlock2(emb_dim=32,adj=self.adj,dropout=self.dropout,act=self.act)
        # self.residual3 = self.residual_block3(R_branch_2)

        # self.layer2 = tf.keras.layers.Dense(units=32, activation=tf.nn.relu)
        # R_branch_3 = self.layer2(self.residual3)
        # self.residual_block4 = ResidualBlock2(emb_dim=32, adj=self.adj, dropout=self.dropout, act=self.act)
        # self.residual3 = self.residual_block4(R_branch_3)
