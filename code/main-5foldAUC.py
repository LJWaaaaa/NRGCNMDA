...
def cross_validation_experiment(drug_dis_matrix, drug_matrix, dis_matrix, seed, epochs, emb_dim, dp, lr, adjdp):
    ...
    all_fprs = []  # 存储所有折叠的FPR值
    all_tprs = []  # 存储所有折叠的TPR值
    ...
    for k in range(k_folds):
        ...
        fpr, tpr, _ = roc_curve(drug_dis_matrix.flatten(), predict_y_proba.flatten())
        all_fprs.append(fpr)
        all_tprs.append(tpr)
    ...
    return metric,all_fprs, all_tprs
...
if __name__ == "__main__":
    ...
    for i in range(circle_time):
        metric, all_fprs, all_tprs= cross_validation_experiment(
        ...
        plt.plot(mean_fpr, mean_tpr, label='Fold %d (AUC = %0.2f)' % (i + 1, mean_auc))
